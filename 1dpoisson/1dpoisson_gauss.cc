...
template<typename T>
int genvecb(float xo,float xn,float yo,float yn, float h,T& b)
{
  int n=0;
  std::vector<float> x;
  while(xo<(xn+h))
...
void plot(float& xo,float& xn,float& yo,float &yn,float h,const std::vector<float>& u)
{
  std::ofstream fout;
...
int main()
{
  ...
  std::cout<<"U after gauss siedel:"<<std::endl;
